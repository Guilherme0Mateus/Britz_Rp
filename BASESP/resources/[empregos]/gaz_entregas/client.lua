...
func = Tunnel.getInterface("gaz_entregas")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
local servico = false
local selecionado = 0
local CoordenadaX = -43.68803024292 -- -43.68803024292,-2520.263671875,7.3947606086731
local CoordenadaY = -2520.263671875
local CoordenadaZ = 7.3947606086731
-----------------------------------------------------------------------------------------------------------------------------------------
-- RESIDENCIAS
-----------------------------------------------------------------------------------------------------------------------------------------
local locs = {
		...
		if servico then
			...
			if distance <= 30.0 then
				...
				if distance <= 2.5 then
					drawTxt("PRESSIONE  ~b~E~w~  PARA ENTREGAR O GAZ",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) then
						if IsVehicleModel(GetVehiclePedIsUsing(ped),GetHashKey("picador")) or IsVehicleModel(GetVehiclePedIsUsing(ped),GetHashKey("tribike3")) then
