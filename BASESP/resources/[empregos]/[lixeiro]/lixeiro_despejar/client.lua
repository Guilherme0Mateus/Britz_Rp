	...
	while true do
		...
		if not processo then
			...
			if distancia <= 3 and GetEntityModel(vehicle) == -1255698084 then
				idle = 5
				DrawMarker(21,-329.45,-1566.93,25.23-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,0,0,50,0,0,0,1)
				if distancia <= 1.5 then
					daleminhapikatxt("PRESSIONE  ~r~E~w~  PARA DESPEJAR O LIXO",4,0.5,0.93,0.50,255,255,255,180)
		...
		if processo then
			daleminhapikatxt("AGUARDE ~g~"..segundos.."~w~ SEGUNDOS ATÉ FINALIZAR O DESPEJO DO LIXO",4,0.5,0.93,0.50,255,255,255,180)
		end
		Citizen.Wait(idle)
	end
end)
