...
job = Tunnel.getInterface("emp_sedex")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('emp_sedex:permissao')
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(ped), -424.05,-2789.53,6.39,true) <= 30 then
		    ...
		    DrawText3D(-424.05,-2789.53,6.39+0.33, "~b~Sedex", 0.9, 1)
		    ...
		    if GetDistanceBetweenCoords(GetEntityCoords(ped), -424.05,-2789.53,6.39,true) <= 1 then
                if IsControlJustPressed(0,38) then	
				    TriggerServerEvent('emp_sedex:permissao')
				end
			end
		end
		...
		if emservico and parte == 1 then
			...
			if pegando and distance2 <= 1.5 and not IsPedInAnyVehicle(ped) then
                if model == -1743316013 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
				...
				if distance2 < 1.5 and not IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0,38) then
                    	if quantidade >= 0 then
                            ...
                            vRP._DeletarObjeto()
                            ClearPedTasksImmediately(ped)
                            TriggerServerEvent("emp_sedex:receber",pagamento)
                            portaaberta = false
                            RemoveBlip(blip)
                        	if quantidade > 0 then
...
function spawnVan()
	...
	while not HasModelLoaded(mhash) do
		...
		nveh = CreateVehicle(mhash,-413.61,-2794.34,5.90+0.5,313.70,true,false)
...
function FadeRoupa(time,tipo,idle_copy)
	...
	if tipo == 1 then 
		...
		TriggerServerEvent("emp_sedex:roupa")
	end
	DoScreenFadeIn(800)
end
...
function CaixaVan(veh,model)
	if quantidade <= 10 then
		...
		if quantidade == 1 and model == -1743316013 then
			propcaixa = nil
			propcaixa = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa,veh,0.0,-0.25,-2.25,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa,true)
		elseif quantidade == 2 and model == -1743316013 then
			propcaixa2 = nil
			propcaixa2 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa2,veh,0.0,0.25,-2.25,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa2,true)
		elseif quantidade == 3 and model == -1743316013 then
			propcaixa3 = nil
			propcaixa3 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa3,veh,0.0,-0.25,-1.80,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa3,true)
		elseif quantidade == 4 and model == -1743316013 then
			propcaixa4 = nil
			propcaixa4 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa4,veh,0.0,0.25,-1.80,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa4,true)
		elseif quantidade == 5 and model == -1743316013 then
			propcaixa5 = nil
			propcaixa5 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa5,veh,0.0,-0.25,-1.35,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa5,true)
		elseif quantidade == 6 and model == -1743316013 then
			propcaixa6 = nil
			propcaixa6 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa6,veh,0.0,0.25,-1.35,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa6,true)
		elseif quantidade == 7 and model == -1743316013 then
			propcaixa7 = nil
			propcaixa7 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa7,veh,0.0,-0.25,-0.9,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa7,true)
		elseif quantidade == 8 and model == -1743316013 then
			propcaix8 = nil
			propcaixa8 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa8,veh,0.0,0.25,-0.9,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa8,true)
		elseif quantidade == 9 and model == -1743316013 then
			propcaixa9 = nil
			propcaixa9 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa9,veh,0.0,-0.25,-0.45,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
			FreezeEntityPosition(propcaixa9,true)
		elseif quantidade == 10 and model == -1743316013 then
			propcaix10 = nil
			propcaixa10 = CreateObject(GetHashKey("hei_prop_heist_box"),coords.x,coords.y,coords.z,true,true,true)
			AttachEntityToEntity(propcaixa10,veh,0.0,0.25,-0.45,-0.05,0.0,0.0,0.0,false,false,true,false,2,true)
