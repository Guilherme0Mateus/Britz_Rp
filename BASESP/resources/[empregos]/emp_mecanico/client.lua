...
Citizen.CreateThread(function()
	while true do 
		...
		local distance = GetDistanceBetweenCoords(GetEntityCoords(ped), -1094.96,-1887.64,16.99, true)
		if distance <= 2 and not servico then
			DrawText3D(-1094.96,-1887.64,16.99,"Pressione ~r~[E]~w~ para inciar o Expediente")
			DrawMarker(23, 1094.96,-1887.64,16.99-1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 66, 32, 8, 100, 0, 0, 0, 0)
		...
		elseif distance <= 2 and servico then  
			DrawMarker(23, 1094.96,-1887.64,16.99-1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 66, 32, 8, 100, 0, 0, 0, 0)
			DrawText3D(-1094.96,-1887.64,16.99,"Pressione ~r~[F7]~w~ para sair de Serviço")
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do 
		Citizen.Wait(1)
		local ped = PlayerPedId()
		local distance = GetDistanceBetweenCoords(GetEntityCoords(ped), 950.53,-977.56,39.76, true)
		if distance <= 2 and not servico then
			DrawText3D(950.53,-977.56,39.76,"Pressione ~r~[E]~w~ para inciar o Expediente")
			DrawMarker(23, 950.53,-977.56,39.76-1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 66, 32, 8, 100, 0, 0, 0, 0)
			if IsControlJustPressed(0, 38) and not servico and txg.checkPermission() then 
				servico = true
				TriggerEvent("Notify", "sucesso", "Você entrou em serviço")
			end
		elseif distance <= 2 and servico then  
			DrawMarker(23, 950.53,-977.56,39.76-1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 66, 32, 8, 100, 0, 0, 0, 0)
			DrawText3D(950.53,-977.56,39.76,"Pressione ~r~[F7]~w~ para sair de Serviço")
		end
	end
end)
