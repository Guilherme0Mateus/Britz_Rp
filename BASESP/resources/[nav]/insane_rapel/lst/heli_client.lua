...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("samumav")
local maverick_hash = GetHashKey("maverick")
local as350bope_hash = GetHashKey("as350bope")
local as350rota_hash = GetHashKey("as350rota")
local as350_hash = GetHashKey("as350")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(5)
		if IsPlayerInPolmav() or IsPlayerInMaverick() or IsPlayerInAs350bope() or IsPlayerInAs350rota() or IsPlayerInAs350() then
...
function IsPlayerInMaverick()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, maverick_hash)
end

function IsPlayerInAs350bope()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, as350bope_hash)
end

function IsPlayerInAs350rota()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, as350rota_hash)
end

function IsPlayerInAs350()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, as350_hash)
end
