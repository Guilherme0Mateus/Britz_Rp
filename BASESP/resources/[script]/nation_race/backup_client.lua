...
fclient = {}
Tunnel.bindInterface("nation_race",fclient)
func = Tunnel.getInterface("nation_race")
						...
						if GetPedInVehicleSeat(vehicle,-1) == ped then
							...
							if distance < 1.5 then
								...
								if IsControlJustPressed(0,38) then
									if func.checkPermission() and not func.getLobby() then
										if func.checkCorredor(k) and func.setValor(k) then
											indexcorrida = func.getIndex()
				...
				if distance < 30 and IsPedInAnyVehicle(ped) then
					...
					if GetPedInVehicleSeat(vehicle,-1) == ped then
						...
						if distance < 3 then
							...
							if IsControlJustPressed(0,38) then
								if func.getLobby() then
									data = func.checkCorredor(func.getIndex())
									if data then
										if func.getValor() then
											indexcorrida = func.getIndex()
										end
									end
								else
			...
			if distance < 100 then
				...
				if distance < 10 then
					...
					if checkpoint < #corrida.checkpoints then
						...
						CriandoBlip(x,y,z)
						func.refreshPos(checkpoint,tempocorrida)
					else
						func.refreshPos(checkpoint,tempocorrida)
						local tempofinal = corrida.tempo - tempocorrida
						func.checkReward(tempofinal,indexcorrida)
						meurank = posicao
						nome = func.getNickName()
						seutempo = calculateTempo(tempofinal)
						melhortempo = calculateTempo(func.getMyRank(indexcorrida))
						ranking = func.getGlobalRank(indexcorrida)
						showStats()
						fclient.finishCorrida()
					end
				end
			end
		end
...
function fclient.startingRace()
	if corredor then
		starting = true
		DoScreenFadeOut(1000)
		Citizen.Wait(1500)
		local vehicle = GetVehiclePedIsIn(PlayerPedId(),false)
		local cds,h = func.getVehiclePosition()
		TeleportPlayer(vehicle,cds[1],cds[2],cds[3])
...
function fclient.finishCorrida()
	lobby = false
	corredor = false
	nrocorredores = nil
	tempocorrida = nil
...
function fclient.notifyOrganizador(type,message)
	if organizador then
		TriggerEvent("Notify",type,message)
	end
end

function fclient.notifyCorredores(type,message)
	if corredor then
		TriggerEvent("Notify",type,message)
	end
end

function fclient.updateOrganizador(status)
	organizador = status
end

function fclient.updateNroCorredores(nro)
	nrocorredores = nro
end

function fclient.updateLobby(status)
	lobby = status
end

function fclient.updateCorredor(status)
	corredor = status
end

function fclient.updateCorrida(status)
	corrida = status
end

function fclient.updateTempo(t)
	if corredor then
		tempocorrida = t
	end
end

function fclient.updateTimer(t)
	timer = t
end

function fclient.isCorredor()
	return corredor
end

function fclient.isOrganizador()
	return organizador
end

function fclient.removeBlips()
	RemoveBlip(blip)
end

function fclient.updateIndex(i)
	indexcorrida = i
end

function fclient.updatePosition(p,p2)
	if corredor then
		posicao = p
		posicoes = p2
	end
...
function fclient.updateEscolhido(bool)
	escolhido = bool
end

function fclient.denuncia()
