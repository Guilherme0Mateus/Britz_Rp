...
const sendData = (name,data) => {
	$.post("http://nation_garages/"+name,JSON.stringify(data),function(datab){});
}

const updateGarages = () => {
		$.post('http://nation_garages/myVehicles',JSON.stringify({}),(data) => {
			...
			$('#inferior-part').html(`
				...
				<div class="item" data-typecar='${item.type}' data-status='${item.status}' data-ipva='${item.ipva}' data-name="${item.name}" data-item-name="${item.name}" data-name2="${item.name2}" data-engine="${item.engine}" data-body="${item.body}" data-fuel="${item.fuel}">
					...
					<div id="backgrounded-car" style="background-image: url(images/${item.name}.png);"></div>
				</div>
			...
			$("#mostraFoto").css("background-image",`url(images/${$(vehicles[0]).data("name")}.png)`)
...
$(document).on('click','.item',function(){
	...
	$("#car-selected").fadeOut(0).delay(400).css("background-image",`url(images/${$(this).data("name")}.png)`).fadeIn(400);
...
$(document).on('click','.retirar',function(){
	let $el = $('.item.active');
	if($el) {
		$.post('http://nation_garages/spawnVehicles',JSON.stringify({
...
$(document).on('click','.guardar',function(){
	$.post('http://nation_garages/deleteVehicles',JSON.stringify({}));
})
