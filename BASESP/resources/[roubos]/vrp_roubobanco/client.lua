...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        IsPerto = false
        local locale = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),255.74096679688,215.16705322266,106.28684234619,true) 
        if locale <= 10 then
            IsPerto = true
            for k, v in ipairs(Config.BankRobbery) do
                ...
                if Distancia < 6.0 then
                    ...
                    TextoMarker(x,y,z+1.0, "APERTE ~r~[ F ]~w~ PARA INICIAR O ROUBO", Opacidade, 0.54, 0.54)
                        DrawMarker(Config.marker.idmarker, x,y,z+0.6, 0, 0, 0, 0, 0, 0, Config.marker.x1,Config.marker.y1,Config.marker.z1,Config.marker.r,Config.marker.g,Config.marker.b,Config.marker.a, Config.marker.pula, 0, 0, Config.marker.gira)
                    if contador == 0 then
                        ...
                        if (IsControlJustPressed(1,49)) then
                            if RBserver.copsonline() >= v.Cops then
                                ...
                                RouboIniciado = true
                                contador = 10 --7200
                                TriggerServerEvent('logsuricato')
                            else
