        ...
        for l,w in pairs(soldado) do
            ...
            if player then
                async(function()
                    ...
                    print('ROUBO AO BANCO INICIADO PELO ID: ' .. user_id)
                end)
            end
        end
    end
...
function vRPrb.permissao()
    local source = source
    local user_id = vRP.getUserId(source)
    return vRP.terPemissao(user_id, "policia.permissao")
end
...
RegisterServerEvent('logsuricato')
AddEventHandler('logsuricato', function()
    local source = source
    local user_id = vRP.getUserId(source)
    SendWebhookMessage(webhookbankrobbery,'O ID: ' .. user_id .. ' INICIOU UM ROUBO AO BANCO CENTRAL.')
end)
