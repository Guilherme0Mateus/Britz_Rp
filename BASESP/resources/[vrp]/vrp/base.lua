...
function vRP.dropPlayer(source)
	...
	if user_id then
		if user_id and source then
			...
			if vRP.hasGroup(user_id,"dpla") then
			...
			elseif vRP.hasGroup(user_id,"dmla") then
		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("Envie SEU ID APROVAMOS NA HORA https://discord.gg/39TaKUd: ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido da cidade ENTRE EM NOSSO https://discord.gg/39TaKUd ")
				--print(user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
