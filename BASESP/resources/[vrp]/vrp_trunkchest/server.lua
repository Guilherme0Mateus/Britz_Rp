...
function vRPN.storeItem(itemName,amount)
	...
	if itemName then
		...
		if user_id then
			if string.match(itemName,"dinheirosujo") or string.match(itemName,"dolar") or string.match(itemName,"mochila") then
				TriggerClientEvent("Notify",source,"negado",'Negado',"Não é possível guardar isto no veículo.")
				return
			end
