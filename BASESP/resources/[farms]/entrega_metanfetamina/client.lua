...
emp = Tunnel.getInterface("entrega_metafetamina")
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1430.13,-749.24,72.63,true) <= 1 then
			DrawText3Ds(1430.13,-749.24,72.63+0.5,"PRESSIONE ~r~E~w~ PARA COMEÇAR MISSÃO")
		...
		if emservico then
			...
			if statuses then
				...
				drawTxt(ui.right_x+0.050,ui.bottom_y-0.058,1.0,1.0,0.45,"ENTREGUE ~g~"..quantidade.."~w~ METAS",255,255,255,255)
			else
			...
			if distance <= 50 then
				...
				if distance < 3 then
                    ...
                    if IsControlJustPressed(0,101) then
                        ...
                        RemoveBlip(blip)
                        TriggerServerEvent('entrega_metafetamina:itensReceber', quantidade)
                        while true do
