...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("insane-character:characterCreate",source)
	end
end

RegisterServerEvent("insane-character:finishedCharacter")
AddEventHandler("insane-character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("insane-character:normalSpawn",source,firstspawn)
